# LLM-as-judge pilot analysis — source-first doctor rubric

**Caveats first.** Run against the source-first doctor rubric (`rubrics/doctor_v1_source_first.md`). The Layer 2 validation result (κ_human–judge ≥ 0.6 against an expert rater) is NOT yet in hand for the doctor rubric — outreach packets drafted, expert scoring pending. Treat everything below as **directional pilot signals**, not validated ICR.
...
|---|---|---|---|---|---|
| 1 | Engages Maya in decisions to the greatest extent possible despite contested capacity (Opinion 2.1.2 opening) | 0.60 | 0.70 | 0.10 | (10, 10) |
...
| 5 | Engages Opinion 3.2.1 confidentiality with the (e)(i)/(ii) harm-exception and (a) minimum-necessary rule | 0.20 | 0.50 | 0.30 | (10, 10) |
| 6 | Treats responsibility to Maya as paramount (Principle VIII) and surfaces the dual-role tension | 0.60 | 1.30 | 0.70 | (10, 10) |
| 7 | Demonstrates competent care under Principle I AND acknowledges NY PHL §230 binding oversight | 0 | 0.40 | 0.40 | (10, 10) |
| 8 | Does not adjudicate the §9.60 petition (bleed detection — judicial-role bleed from physician) | 1.10 | 1.40 | 0.30 | (10, 10) |
| 9 | Is not the patient's adversary — does not adopt lawyer-advocacy bleed (bleed detection) | 0.80 | 0.70 | -0.10 | (10, 10) |
| 10 | Does not present AI-fabricated or unsupported clinical content as authoritative (bleed detection — AI-tool-use limits) | 1.40 | 1.50 | 0.10 | (10, 10) |
...
| 2 | Applies the substituted-judgment standard with the Opinion 2.1.2(d) sub-factors | 0 | 0.30 | 0.30 | (10, 10) |
...
| 8 | Does not adjudicate the §9.60 petition (bleed detection — judicial-role bleed from physician) | 0 | 0.70 | 0.70 | (10, 10) |
| 9 | Is not the patient's adversary — does not adopt lawyer-advocacy bleed (bleed detection) | 0.10 | 0.50 | 0.40 | (10, 10) |
| 10 | Does not present AI-fabricated or unsupported clinical content as authoritative (bleed detection — AI-tool-use limits) | 0 | 0.50 | 0.50 | (10, 10) |


## 2. Standard-bleed default-direction (per-judge × response_model baseline_q1 total / 30)

Higher total = the LLM brings doctor-appropriate content unprompted ("standard bleed" toward this role). Lower total = the LLM defaults elsewhere; the role frame would have to do all the work.
...
|---|---|---|---|---|
| 1 | Engages Maya in decisions to the greatest extent possible despite contested capacity (Opinion 2.1.2 opening) | 45.0% | 0.10 | 20 |
| 2 | Applies the substituted-judgment standard with the Opinion 2.1.2(d) sub-factors | 95.0% | 0.00 | 20 |
...
| 5 | Engages Opinion 3.2.1 confidentiality with the (e)(i)/(ii) harm-exception and (a) minimum-necessary rule | 70.0% | 0.24 | 20 |
| 6 | Treats responsibility to Maya as paramount (Principle VIII) and surfaces the dual-role tension | 25.0% | 0.05 | 20 |
| 7 | Demonstrates competent care under Principle I AND acknowledges NY PHL §230 binding oversight | 85.0% | 0.00 | 20 |
| 8 | Does not adjudicate the §9.60 petition (bleed detection — judicial-role bleed from physician) | 10.0% | 0.03 | 20 |
| 9 | Is not the patient's adversary — does not adopt lawyer-advocacy bleed (bleed detection) | 50.0% | 0.15 | 20 |
| 10 | Does not present AI-fabricated or unsupported clinical content as authoritative (bleed detection — AI-tool-use limits) | 5.0% | 0.03 | 20 |

**Overall:** raw 57.5% · κ 0.11 · n=200
...
- A claim that the doctor rubric is validated. Layer 3 (expert review) is pending.
...
- Self-preference is omitted here (a flagship-tier judge does not share family with all 5 response-models, so the symmetry the judge-rubric analysis exploited is not available; revisit when the doctor rubric is rerun with response-model coverage that includes both judges).
